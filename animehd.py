...
def get_servers(html, resolution_key):
    ...
    for li in li_list:
        ...
        links = li.find_all('a')
        dict_of_servers[server_name] = [
            f'{link["href"]}download/{resolution[resolution_key]}' for link in links]
...
def video_exists(destination_path, file_name, link):
    ...
    if file_name in file_list:
        # check size
        response = requests.head(link, stream=True, headers=headers)
...
if __name__ == "__main__":
    ...
    # download("https://op.manga47.net/One_Piece_Dub/039.MP4", "One_Piece_039")

    start_at = 36

    while start_at < 100:
        video_name = f'One_Piece_0{start_at}.mp4'
        video_link = f'https://op.manga47.net/One_Piece_Dub/0{start_at}.MP4'
        start_at += 1
        if video_exists(destination_path, video_name, video_link):
            print(f"Video {video_name} exists")
            continue

        download(video_link, video_name)

    # video_exists(destination_path, 'One_Piece_039.mp4',
    #              "https://op.manga47.net/One_Piece_Dub/039.MP4")
